...
<html lang="en">
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
      <ul class="navbar-nav"> 
        ...
        <li class="nav-item">
          <a class="nav-link ms-5" href="datatransaksi.php">Transaksi</a>
        </li>
  ...
  <h3 class="mt-4" style="margin-left: 29%;"> Data Transaksi </h3>

<table class="border-atas"border="1">
    <tr>
        <th>No</th>
        <th>id_order</th>
        <th>total_item </th>
    ...
    $no=1;
    $ambildata = mysqli_query($conn,"select * from transaksi_paket");
    while($tampil = mysqli_fetch_array($ambildata)){
        echo "
        <tr>
            <td>$no</td>
            <td>$tampil[id_order]</td>
            <td>$tampil[total_item]</td>
            <td>$tampil[total_bayar]</td>
            <td>$tampil[tgl_transaksi]</td>
            
            <td><a href='?kode=$tampil[id_order]'> Hapus </a></td>
        <tr>";
        $no++;
    }
    ?>
      ...
      if(isset($_GET['kode'])){
        mysqli_query($conn,"delete from transaksi_paket where id='$_GET[kode]'");
