...
if (isset($_POST['id_produk'], $_POST['pembelian'])) {
 ...
 $produk = mysqli_query($conn, "SELECT * FROM paket WHERE id = '$id'");
 ...
 if ($index == -1) {
  $_SESSION['cart'][] = [
   'id_produk' => $id,
   'nama_paket' => $dt_produk['nama_paket'],
   'harga_paket' => $dt_produk['harga_paket'],
   'pembelian' => $pembelian
  ];
 }
}
 ...
 ?>
  ...
  <table class="table table-bordered">
   ...
   if(isset($_SESSION['cart'])) {
    ...
    for ($i=0; $i<count($cart); $i++) {
     $total = $_SESSION['cart'][$i]['harga_paket'] * $_SESSION['cart'][$i]['pembelian'];
     $total_bayar += $total;
     ?>

     <tr>
      <td align="center"><?= $no++; ?></td>
      <td><?= $cart[$i]['nama_paket']; ?></td>
      <td align="center"><?= $cart[$i]['pembelian']; ?></td>
      <td><?= $cart[$i]['harga_paket']; ?></td>
      <td><?= $total; ?></td>
      <td align="center">
       <a href="?index=<?= $index; ?>">
